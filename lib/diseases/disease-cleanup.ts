...
export class DiseaseCleanupProcessor {
  constructor(private readonly prisma: PrismaClient = getPrismaClient(), private readonly storage: EntityImageStorage = getEntityImageStorage("diseases")) {}
  ...
  async queueOrphans() { const [keys, images] = await Promise.all([this.storage.listKeys(), this.prisma.diseaseImage.findMany({ select: { storageKey: true } })]); const known = new Set(images.map((image) => image.storageKey)); const orphans = keys.filter((key) => !known.has(key)); await Promise.all(orphans.map((key) => this.queue(key))); return orphans.length; }
}
