...
type DeleteDiseaseButtonProps = {
  diseaseId: string;
  diseaseName: string;
  onDeleted?: (diseaseId: string) => void;
};
  ...
  async function deleteDisease() {
    ...
    try {
      const response = await fetch(
        `/api/admin/diseases/${encodeURIComponent(
          diseaseId,
        )}`,
        {
          method: "DELETE",
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "حدث خطأ غير متوقع أثناء حذف المرض.",
      );
    } finally {
      setDeleting(false);
    }
    ...
    <>
      ...
      {open ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby={`delete-disease-title-${diseaseId}`}
        >
          <div className="w-full max-w-lg rounded-3xl border border-red-400/20 bg-[#0b2118] p-5 shadow-2xl sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2
                    id={`delete-disease-title-${diseaseId}`}
