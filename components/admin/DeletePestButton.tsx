...
type DeletePestButtonProps = {
  pestId: string;
  pestName: string;
  onDeleted?: (pestId: string) => void;
};

type DeletePestResponse = {
  success?: boolean;
  message?: string;
  error?: string;
};

export default function DeletePestButton({
  pestId,
  pestName,
  onDeleted,
}: DeletePestButtonProps) {
  const [open, setOpen] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [errorMessage, setErrorMessage] =
    useState("");

  async function deletePest() {
    if (deleting) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/admin/pests/${encodeURIComponent(
          pestId,
        )}`,
        {
          method: "DELETE",
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "حدث خطأ غير متوقع أثناء حذف الآفة.",
      );
    } finally {
      setDeleting(false);
    }
    ...
    <>
      ...
      {open ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby={`delete-pest-title-${pestId}`}
        >
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm leading-7 text-white/55">
                    ...
                    وكل البيانات التابعة لها التي تسمح قاعدة
                    البيانات بحذفها.
                  </p>
                </div>
              </div>
