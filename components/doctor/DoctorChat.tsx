...
export function DoctorChat() {
  ...
  return <section className="grid gap-6 lg:grid-cols-[minmax(260px,0.36fr)_minmax(0,1fr)]" aria-label="محادثة دكتور ArtVert">
    <div className="space-y-4 lg:sticky lg:top-6 lg:self-start"><DoctorCharacter state={characterState} /><section className="rounded-2xl border border-green-700/60 bg-black/20 p-4"><h2 className="font-black text-lime-200">كيف يساعدك الطبيب؟</h2><p className="mt-2 text-sm leading-7 text-green-50/75">اكتب اسم النبات وأهم عرض ظاهر. لا نطلب صوراً في هذه المرحلة.</p></section></div>
    <div className="rounded-3xl border border-green-700/60 bg-[#161b17]/95 p-4 shadow-2xl shadow-black/20 sm:p-6">
      ...
      <div className="mt-5 space-y-4" aria-label="سجل المحادثة">
        {transcript.map((item) => item.role === "user" ? <article key={item.id} className="mr-auto max-w-[88%] rounded-2xl rounded-tr-sm bg-lime-300 px-4 py-3 text-sm font-bold leading-7 text-[#152015]">{item.text}</article> : <div key={item.id} ref={item.id === newestResponseId ? newestResponseRef : undefined} tabIndex={-1} className="rounded-2xl border border-green-700/60 bg-[#1b2820] p-5 outline-none focus-visible:ring-2 focus-visible:ring-lime-200"><DoctorResult result={item.result!} />{item.result?.plant.alternatives.length ? <section className="mt-5"><h2 className="font-black text-lime-200">هل تقصد أحد هذه النباتات؟</h2><div className="mt-3 flex flex-wrap gap-2">{item.result.plant.alternatives.map((alternative) => <button type="button" key={alternative.id} onClick={() => selectPlant(alternative.name)} disabled={Boolean(requestRef.current)} className="rounded-xl border border-lime-300/60 px-4 py-2 text-sm font-bold text-lime-100 transition hover:bg-lime-300 hover:text-[#152015] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-lime-100 disabled:opacity-50"><bdi>{alternative.name}</bdi></button>)}</div></section> : null}</div>)}
