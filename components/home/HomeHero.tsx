  ...
  return <>
    <main className="home-hero min-h-screen bg-[#071109] text-white" dir="rtl">
      <section className="relative isolate min-h-[870px] overflow-hidden border-b border-lime-300/15 bg-[#071109]">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-6 px-4 pb-32 pt-8 sm:px-6 lg:grid-cols-[.85fr_1.4fr_.42fr] lg:items-center lg:pt-16">
          <div className="order-2 mx-auto w-full max-w-[360px] lg:order-1 lg:mx-0"><div className="relative"><div className="absolute left-0 top-8 rounded-2xl border border-lime-200/30 bg-[#f5ffe7] px-4 py-3 text-sm font-bold text-[#14351d] shadow-xl before:absolute before:-left-2 before:top-7 before:border-y-8 before:border-r-10 before:border-y-transparent before:border-r-[#f5ffe7]">أهلاً بك! كيف أساعد نباتك اليوم؟</div><DoctorAvatarAsset state="WELCOME" className="pt-20"/></div></div>
          <div className="order-1 text-right lg:order-2"><p className="mb-5 text-sm font-black tracking-[.18em] text-lime-300">حلول زراعية ذكية من قلب الأرض</p><h1 className="max-w-3xl text-5xl font-black leading-[1.16] sm:text-6xl xl:text-7xl">نغذّي نباتك<br/><span className="text-lime-300">ونفهم احتياجاته.</span></h1><p className="mt-6 max-w-2xl text-lg leading-8 text-white/75">منتجات زراعية موثوقة، خبرة عملية، ودكتور ArtVert لمساعدتك في اتخاذ الخطوة التالية بثقة.</p><div className="mt-8 flex flex-wrap gap-3"><Link href="/doctor" className="min-h-12 rounded-xl bg-lime-300 px-6 py-3 font-black text-[#0a2511] shadow-[0_12px_32px_rgba(178,255,48,.24)] transition hover:bg-lime-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-lime-100">اسأل دكتور ArtVert</Link><Link href="/products" className="min-h-12 rounded-xl border border-white/25 bg-white/5 px-6 py-3 font-bold backdrop-blur transition hover:bg-white/15">تصفح المنتجات</Link></div>
          ...
          <aside className="order-3 grid grid-cols-2 gap-2 lg:grid-cols-1" aria-label="خدمات ArtVert">{services.map(([name, href], index) => <Link key={href} href={href} className="group min-h-16 rounded-2xl border border-white/10 bg-[#0c2112]/75 p-3 text-center text-xs font-black text-white shadow-xl backdrop-blur transition hover:border-lime-300/60 hover:bg-[#183a20]"><span className="mb-1 block text-lime-300">0{index + 1}</span>{name}</Link>)}</aside>
        </div>
